...
def set_config():
    with patch('saeapi.stage.SaeApiConfig') as mock_config:
        def _make_mock_config(stream_id: str, input_stream_prefix: str):
            mock_config.return_value = SaeApiConfig(
                log_level='WARNING',
                redis=RedisConfig(
                    stream_id=stream_id,
...
def redis_publisher_mock():
    with patch('saeapi.stage.ValkeyPublisher') as mock_publisher:
        yield mock_publisher.return_value.__enter__.return_value

@pytest.fixture
def inject_consumer_messages():
    with patch('saeapi.stage.ValkeyConsumer') as mock_consumer:
...
def test_example(set_config, redis_publisher_mock, inject_consumer_messages):
    ...
    assert redis_publisher_mock.call_args_list[0].args == ('saeapi:test_stream', _make_sae_msg_bytes(1))
    assert redis_publisher_mock.call_args_list[1].args == ('saeapi:test_stream', _make_sae_msg_bytes(2))
